...
namespace array
{
    namespace detail
    {
        template <typename ViewT, typename IterT>
        struct compatible_value_type : std::false_type
        {};
        template <typename T>
        struct compatible_value_type<T, T> : std::true_type
        {};
        template <typename T>
        struct compatible_value_type<const T, T> : std::true_type
        {};

        template <typename ViewT, typename IterT>
        using require_compatible_value_type =
            typename std::enable_if<compatible_value_type<ViewT, IterT>::value>::type;
    } // namespace detail
    ...
    ///
    /// A type is a block if it has a valid specialization and is convertible to an
    /// [array::block_view](). The traits must be specialized for `T` and `const T` to allow
    /// propagating the const-ness.
    ///
