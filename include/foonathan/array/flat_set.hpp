...
namespace array
{
    ...
    ///
    /// Use this in [array::flat_set]() if you want to have an [array::flat_map]() where the values
    /// are stored together with the keys. Only the `key` is being compared, not the value.
    ...
    struct key_value_pair
    {
        ...
        /// \effects Creates it by constructing the key from the given key and the value from the
        /// arguments.
    ...
    /// you have something similar to [array::flat_map]() but where the keys and values are stored
    /// together.
    ...
    public:
        ...
        operator sorted_view<Key, Compare>() const noexcept
        {
            return sorted_view<Key, Compare>(array_);
        }
        ...
        /// \effects Does a lookup of the given key.
        /// If the key is not already in the set, inserts the key constructed by forwarding `key`
        /// followed by `args`. Otherwise, does nothing. \returns The result of the insert
        /// operation. \requires The set is a multiset. \notes This function is the same as
        /// `emplace()` on a non-multiset.
        template <typename TransparentKey, typename... Args>
        ...
        /// \effects Does a lookup of the given key.
        /// If the key is not already in the set, inserts the key constructed by forwarding `key`
        /// followed by `args`. Otherwise, replaces the key with the key constructed by forwarding
        /// `key` followed by `args`. \returns The result of the insert operation. \requires The set
        /// is not a multiset and the new key must be inserted in the same position as the old key.
        /// The latter is guaranteed if the transparent key argument identifies the part of the key
        /// that is actually being compared, and the other arguments are ignored.
        template <typename TransparentKey, typename... Args>
        insert_result emplace_or_replace(TransparentKey&& key, Args&&... args)
        {
            static_assert(sizeof(TransparentKey) == sizeof(TransparentKey) && !AllowDuplicates,
                          "emplace_or_replace doesn't make sense on multiset");
            auto result = emplace(std::forward<TransparentKey>(key), std::forward<Args>(args)...);
        ...
        /// constructible from `K`. \group insert \param 1 \exclude
        template <typename K,
        ...
        {
            insert_range_impl(typename std::iterator_traits<InputIt>::iterator_category{}, begin,
                              end);
        }
        ...
        /// \effects Conceptually the same as `flat_set<Key> s; s.insert_range(begin, end); *this =
        /// std::move(s);`
        template <typename InputIt>
