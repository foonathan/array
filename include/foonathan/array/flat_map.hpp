...
namespace array
{
    ...
    namespace detail
    {
        ...
        public:
            ...
            // TODO: non-trivial copy constructor...
            flat_map_iterator(const flat_map_iterator<Key, typename std::remove_const<Value>::type>&
                                  non_const) noexcept
            ...
            {
                return flat_map_iterator(private_key{}, iter.key_ + offset, iter.value_ + offset);
            }
    ...
    /// but uses a [array::flat_set]() for the key storage and an [array::array]() internally, both
    /// using the given `BlockStorage`. Matching keys and values are implicitly linked by being
    ...
    public:
        ...
        {
            return const_iterator(typename const_iterator::private_key{}, iterator_to_pointer(iter),
                                  iterator_to_pointer(value_iter(iter)));
        }
        ...
        {
            return iterator(typename iterator::private_key{}, iterator_to_pointer(key_iter(iter)),
        ...
        const_iterator key_value_iter(value_const_iterator iter) const noexcept
        {
            return const_iterator(typename const_iterator::private_key{},
                                  iterator_to_pointer(key_iter(iter)), iterator_to_pointer(iter));
        }
        ...
        /// \effects Reserves new memory to make capacity as least as big as `new_capacity` if that
        ...
        /// If the key isn't part of the map or the map allows duplicates, inserts the
        /// key-value-pair where the key is constructed from the transparent key and the value is
        /// constructed from the arguments. Otherwise, does nothing. \returns The result of the
        /// insert operation.
        ...
        /// where the key is constructed from the transparent key and the value is constructed from
        /// the arguments. Otherwise, does nothing. \returns The result of the insert operation.
        ...
        /// If the key isn't part of the map or the map allows duplicates, inserts the
        /// key-value-pair where the key is constructed from the transparent key and the value is
        /// constructed from the arguments. Otherwise, assigns the value already stored to the value
        /// constructed by from the arguments. \returns The result of the insert operation.
        ...
        insert_result emplace_or_replace(TransparentKey&& key, ValueArgs&&... args)
        {
            ...
            {
                // replace value
                detail::replace_value(*value_iter(result.iter()), std::forward<ValueArgs>(args)...);
        ...
        /// \notes This function does not participate in overload resolution if `Key` is not
        /// convertible from `K`, or `Value` is not convertible from `V`. \group insert \param 2
        /// \exclude
        ...
        /// \effects Same as the emplace variant being called with `(get<0>(FWD(pair)),
        /// get<1>(FWD(pair))`. \group insert_pair
        template <class Pair>
        ...
        /// \effects Inserts keys from the range `[key_begin, key_end)` combined with the matching
        /// values from `[value_begin, value_end)` as if calling `emplace()`. It will stop as soon
        /// as one range is exhausted.
        ...
        /// \effects Inserts all elements in the range `[begin, end)` by calling
        /// `insert_pair(*cur)`.
        template <typename InputIt>
        void insert_pair_range(InputIt begin, InputIt end)
        {
            reserve(size()
                    + range_size(typename std::iterator_traits<InputIt>::iterator_category{}, begin,
                                 end));
            ...
            typename std::conditional<AllowDuplicates, size_type, bool>::type
        {
            ...
            using result_type = typename std::conditional<AllowDuplicates, size_type, bool>::type;
        ...
        /// \effects Conceptually the same as `flat_map<Key, Value> m; m.insert_range(key_begin,
        /// key_end, value_begin, value_end); *this = std::move(m);`
        template <typename KeyInputIt, typename ValueInputIt>
        ...
        /// \effects Conceptually the same as `flat_map<Key, Value> m; m.insert_pair_range(begin,
        /// end); *this = std::move(m);`
        template <typename InputIt>
        ...
        /// \returns A pointer to the value belonging to the given key, or `nullptr`, if there was
        /// none. \group try_lookup
        template <typename TransparentKey>
        ...
        /// \returns An iterator to the given key-value-pair, or `end()` if the key is not in the
        /// map. \group find
        template <typename TransparentKey>
