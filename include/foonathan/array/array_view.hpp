...
namespace array
{
    ...
    /// \notes If you don't need the special functions like array access, use [array::block_view]()
    /// instead. \notes If you require a view into a *sorted* array, use [array::sorted_view]()
    ...
    public:
        ...
        /// \notes This constructor only participates in overload resolution, if `const U` is the
        /// same as `T`, i.e. `T` is `const` and `U` is the non-`const` version of it.
        template <typename U, typename = detail::require_compatible_value_type<T, U>>
        ...
        template <typename U, typename = detail::require_compatible_value_type<T, U>>
        explicit constexpr array_view(const block_view<U>& block) noexcept : block_view<T>(block)
        {}
