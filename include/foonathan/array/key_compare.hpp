...
namespace array
{
    ...
    struct key_compare_default
    {
        ...
        ///
        /// A specialization for a user-defined type must give it the same interface as required by
        /// the `KeyCompare` concept.
        ///
    ...
    namespace detail
    {
        ...
        public:
            ...
            /// \notes This constructor only participates in overload resolution if `ContIter` is a
            /// contiguous iterator, and the value type is the same.
            template <typename ContIter, typename = detail::require_compatible_value_type<
                                             T, contiguous_iterator_value_type<ContIter>>>
    ...
    /// It is always a view to const, `sorted_view<T>` and `sorted_view<const T>` are the same type.
    ///
    /// \notes If you have a non sorted array, use [array::array_view]() or [array::block_view]()
    /// instead.
    ///
    /// \notes Inheritance is only used to easily inherit all functionality as well as
    /// allow conversion without a user-defined conversion. Slicing is permitted and works, but the
    /// type isn't meant to be used polymorphically. \notes Write an implicit conversion operator
    /// for containers that have contiguous storage with ordering, and specialize the
    /// [array::block_traits]() if it does not provide a `value_type` typedef.
    template <typename T, class Compare = key_compare_default>
    using sorted_view = detail::sorted_view<const T, Compare>;
    ...
    /// \notes This function does not participate in overload resolution, unless they are contiguous
    /// iterators.
    ...
    /// \returns An iterator pointing to the first element that is greater or equal to the given
    /// key. \requires The sequence is sorted according to the key comparison.
